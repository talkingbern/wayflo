// src/TripMap.jsx
// Uses refs exclusively inside Mapbox event handlers to avoid stale closure crashes
...
export default function TripMap({ days, dayPhotos = {} }) {
  const containerRef     = useRef(null);
  const mapRef           = useRef(null);
  const initialBoundsRef = useRef(null);
  const mapReadyRef      = useRef(false);
  const activePinRef     = useRef(null); // source of truth for active pin
  const pinsRef          = useRef([]);   // stable ref to pins data

  const [loaded, setLoaded]     = useState(false);
  const [error, setError]       = useState(false);
  const [activePin, setActivePin] = useState(null); // mirror of activePinRef for rendering

  const pins = (days || [])
    .map((d, i) => ({ ...d, index: i }))
    .filter(d => d.lat && d.lng && !isNaN(parseFloat(d.lat)) && !isNaN(parseFloat(d.lng)));

  // Keep pinsRef in sync
  ...
  function resetView() {
    if (!mapRef.current || !mapReadyRef.current || !initialBoundsRef.current) return;
    try { mapRef.current.fitBounds(initialBoundsRef.current, { padding:50, duration:900 }); } catch(e) {}
    activePinRef.current = null;
    setActivePin(null);
  }

  // Safe click handler — only uses refs, never closes over React state
  function handlePinClickSafe(pinIndex) {
    const newActive = activePinRef.current === pinIndex ? null : pinIndex;
    activePinRef.current = newActive;
    // Schedule React state update outside Mapbox event loop
    requestAnimationFrame(() => {
      setActivePin(newActive);
      if (newActive !== null) {
        const pin = pinsRef.current.find(p => p.index === pinIndex);
        if (pin) flyToPin(pin);
      }
    });
  }

  // Legend pill click — safe to call directly from React
  function handleLegendClick(pin) {
    handlePinClickSafe(pin.index);
  }
  ...
  function initMap() {
    if (!containerRef.current || pinsRef.current.length === 0 || mapRef.current) return;
    try {
      const mapboxgl = window.mapboxgl;
      mapboxgl.accessToken = MAPBOX_TOKEN;

      const currentPins = pinsRef.current;
      const lngs  = currentPins.map(p => parseFloat(p.lng));
      const lats  = currentPins.map(p => parseFloat(p.lat));
      const pad   = currentPins.length === 1 ? 0.05 : 0.4;
      const bounds = [
      ...
      map.on("load", () => {
        ...
        if (currentPins.length > 1) {
          map.addSource("route", {
            type: "geojson",
            data: { type:"Feature", geometry:{ type:"LineString", coordinates:currentPins.map(p=>[parseFloat(p.lng),parseFloat(p.lat)]) } }
          });
        ...
        currentPins.forEach((pin, i) => {
          const isFirst   = i === 0;
          const isLast    = i === currentPins.length - 1;
          const icon      = getTransportIcon(pin, isFirst, isLast);
          const isSpecial = icon !== null;
          const pinIndex  = pin.index; // capture in closure
          ...
          // Use only pinIndex (primitive) in handler — no React state
          el.addEventListener("click", (e) => {
            e.stopPropagation();
            handlePinClickSafe(pinIndex);
          });
  ...
  return (
    <div style={{ marginBottom:"1.5rem" }}>
      ...
      <div style={{
        position:"relative",
        borderRadius: activeDayData ? "12px 12px 0 0" : "12px",
        overflow:"hidden",
        border:"1px solid var(--sand)",
        borderBottom: activeDayData ? "none" : "1px solid var(--sand)",
        height:"280px",
        background:"var(--sand)",
      }}>
        {!loaded && (
        ...
        {loaded && (
          <button onClick={resetView} title="Reset view"
            style={{ position:"absolute", bottom:10, right:10, zIndex:10, background:"white", border:"1px solid #ccc", borderRadius:"6px", width:30, height:30, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", fontSize:"14px", boxShadow:"0 1px 4px rgba(0,0,0,0.2)" }}>
            🏠
          </button>
        )}
      </div>
      ...
      {/* Inline day panel */}
      {activeDayData && (
        <div style={{ background:"var(--white)", border:"1px solid var(--rust)", borderTop:"2px solid var(--rust)", borderRadius:"0 0 12px 12px", overflow:"hidden" }}>
          {dayPhotos[activePin] && (
          ...
          <div style={{ padding:"0.9rem 1.1rem" }}>
            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:"0.5rem" }}>
              ...
              <button onClick={()=>{ activePinRef.current=null; setActivePin(null); }}
            ...
            {activePinData?.locationName && (
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"0.55rem" }}>
                ...
                <a href={"https://www.google.com/maps/dir/?api=1&destination="+encodeURIComponent(activePinData.locationName)}
                  target="_blank" rel="noopener noreferrer"
                  style={{ fontSize:"0.7rem", color:"var(--rust)", textDecoration:"none", display:"flex", alignItems:"center", gap:"0.25rem", fontWeight:500, border:"1px solid var(--rust)", borderRadius:"999px", padding:"0.15rem 0.55rem" }}>
                  Navigate →
                </a>
              </div>
            )}
      ...
      <div style={{ display:"flex", gap:"0.4rem", flexWrap:"wrap", marginTop:"0.6rem" }}>
        {pins.map((pin, i) => {
          ...
          return (
            <button key={i} type="button" onClick={() => handleLegendClick(pin)}
